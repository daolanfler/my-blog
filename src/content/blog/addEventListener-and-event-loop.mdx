...
> 问题：冒泡和原始事件的回调是在同一个 event-loop 中执行的吗？
...
1. 用户点击，触发 `click` 事件
2. 子元素的 _onClick callback_ 执行
3. 响应式数据的副作用同步触发
4. 给父元素 addEventListener (并记录下 attached 的时间)
5. 事件冒泡到父元素
6. 父元素的 _onClick callback_ 执行 (在这里比较 attaced 时间和事件触发的时间，决定是否执行回调)
